...
打造一个面向合成氨装置现场的智能生产调度专家，让调度员在安全边界内快速回答四个问题：
...
补充试题进一步强调：硬件层面的单位生产成本已接近理论最优，真正空间在合成氨运行管理的软件层。因此方案必须直面三类企业痛点：多因素联合优化导致的调度寻优难、人工链条长导致的指挥效率低、专家经验不能沉淀导致的知识传承难。平台不应只是看板，而应具备围绕氨合成负荷、氢氮比、合成塔床层温升、循环压缩机、液氨库存和下游用氨的“实时感知、吨氨成本寻优、方案生成、指挥协同、执行监督、知识沉淀、自我迭代”闭环能力。

## 合成氨超级数字员工能力矩阵

| 企业挑战 | 数字员工能力 | 可操作设计 |
| --- | --- | --- |
| 调度寻优难 | 吨氨成本雷达、稳氨/保供/护机三案比选、合成回路约束孪生 | 实时拆解原料/蒸汽电耗、合成塔窗口、循环压缩机效率、液氨库存、下游消纳的成本挖潜来源 |
| 指挥效率低 | 合成氨调控师指令窗口、审批链、执行监督 | 一键重算、生成交接摘要、标记采纳，自动跟踪氨合成负荷偏差、氢氮比偏差和液氨库存变化 |
| 知识传承难 | 合成负荷指令库、氢氮比/床层温升异常库、未采纳原因标签 | 每次方案、采纳/未采纳原因、执行结果进入知识库，用于周度校准 |
...
- 将优化结果翻译成调度语言。
- 对每个方案给出原因、约束、收益和风险。
- 支持班中情景推演：“如果压缩机状态下降 10%，怎么排？”“如果复合肥订单提前一天呢？”
- 生成班前会摘要、交接班说明和异常复盘。
...
- 数据血缘：每条系统方案都能追溯到输入数据版本、模型版本和约束规则版本。
- 口径对齐：生产口径、财务口径、库存口径和调度口径需要在平台中明确转换关系。

### 7. 模型治理层

- 灰度上线：先离线回放，再影子模式，再人机确认，最后局部闭环。
- 置信阈值：低置信方案只作为备选方案，高风险方案必须双人确认。
- 漂移监控：监控订单预测、设备健康预测、能耗模型和库存模型的偏差。
- 回滚机制：当模型偏差连续超阈值，自动降级到专家规则和人工调度。

### 8. 安全与权限层

- 系统不直接绕过 DCS/SIS/MES 安全边界，只输出方案和经审批的计划。
- 分角色权限：班长看操作口径，调度看排产全局，设备看健康风险，经营看订单收益，安环看红线约束。
- 留痕审计：记录谁采纳、何时采纳、采纳前后关键 KPI、执行偏差和复盘结论。
- 应急降级：网络中断、数据延迟、模型低置信时，平台降级为规则库和人工流程。

### 8.1 飞书协同层

- 飞书机器人把“稳氨/保供/护机”方案推送到合成氨当班调度群，卡片显示目标负荷、风险指数、触发约束和人工确认按钮。
- 飞书审批承载高风险负荷调整的留痕流程，审批通过后也只允许写 MES 计划、交接摘要和复盘记录，不直接写 DCS/SIS。
- 飞书事件回调用于接收审批通过、审批驳回、卡片按钮点击和班后复盘提交，并写入合成负荷指令库、异常经验库和未采纳原因库。
- 真实接入需要企业自建应用 App ID/App Secret、机器人能力、目标群权限、审批定义 approval_code 和事件回调地址。

### 9. 知识库与自学习层

- 合成负荷指令库：记录“输入状态-目标合成负荷-审批人-执行动作-偏差结果”。
- 异常处置经验库：沉淀氢氮比偏差、合成塔床层温升异常、循环压缩机健康下降、液氨库存偏低、下游用氨插单等场景。
...
3. 输出 24 小时排产方案、风险解释和价值测算。
4. 由调度员人工确认，不自动控制装置。
5. 每天复盘预测偏差和调度收益。

## 企业试点务实边界

企业真正关心的不是“智能系统能不能算”，而是“能不能低风险嵌入现有生产组织”。因此 MVP 要把范围压小：

- **先不碰控制层**：不改 DCS/SIS 控制逻辑，不自动开停车，不越过现有审批链。
- **先做薄应用**：复用现有 MES、ERP、DCS historian、罐区和设备点检系统，不重做 MES/ERP。
- **先影子运行**：连续 4-6 周输出方案但不回写，记录调度员是否采纳和原因。
- **先算真实收益**：只统计采纳方案带来的能耗、库存、订单、停车风险改善，不把行情上涨算成系统收益。
...
- **模型可信**：需要置信度、漂移、回放验证和回滚机制，否则系统方案很难被调度员长期采纳。
- **安全可信**：合成氨是高压、高温、高风险连续流程，系统必须服从安全硬约束和人工确认。
...
- **第 2 阶段：离线方案**  
  基于历史数据回放，验证系统方案是否优于经验调度。

- **第 3 阶段：人机协同执行**  
  由系统方案、调度员确认、MES 回写，形成闭环。
...
上述指标在正式立项时采用保守口径：先以历史回放验证方向，再以影子运行验证采纳率，最后只对“被采纳且实际执行”的方案核算收益。

## 当前原型说明

本仓库提供一个静态前端原型，使用滑块模拟订单、能价、设备健康、库存和市场波动。页面会实时计算目标负荷、日产氨、风险指数、价值测算和班次 Gantt 图。它不是控制系统，而是用于方案验证和沟通的交互样机。
