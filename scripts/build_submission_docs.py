...
REFERENCES = [
    ...
    "中国信息通信研究院、华为等. 工业智能融合应用资料. https://www-file.huawei.com/dam/asset/view/dda7ee674ced41d4bb989b365ea0a028.pdf",
...
PART1_SUBMISSION = (
    "这道题的核心不是再造一个看板，而是在合成氨硬件成本已接近极限后，用智能算法挖掘运行管理的软件降本空间。"
...
PART2_SUBMISSION = (
    "我设计“合成氨智能调控师工作台”，先以影子运行接入MES日计划、ERP订单交期、DCS关键点位、液氨罐区、设备点检和能源价格，形成班次事实表。"
    "系统用预测模型滚动判断下游用氨、库存水位、吨氨能耗和设备风险；用机理约束校验氢氮比、合成塔床层温升、循环压缩机负荷、罐区压力、安全库存和负荷升降速率；"
    "再由优化器生成稳氨、保供、护机三案，给出目标负荷、下游分配、成本收益、风险边界和需人工确认事项。"
...
def build_submission_form_doc():
    doc = setup_doc("提交信息汇总", "项目表单、附件、链接与队伍信息")
    doc.add_heading("1. 开题报告 Part 1：命题前置分析与洞察", level=1)
    add_para(doc, f"提交正文（{len(PART1_SUBMISSION)} 字）：", bold_prefix="提交正文")
    add_para(doc, PART1_SUBMISSION)

    doc.add_heading("2. 开题报告 Part 2：整体解决方案设计", level=1)
    add_para(doc, f"提交正文（{len(PART2_SUBMISSION)} 字）：", bold_prefix="提交正文")
    ...
    add_table(
        ...
        [
            ...
            ["整体解决方案书", "说明方案不是概念页，而是有架构、接口、流程、验收和停用边界", "02_整体解决方案书.docx"],
            ["调控师操作手册", "从企业班组视角说明如何用、谁确认、何时降级、如何复盘", "03_调控师操作手册.docx"],
            ["参考文献与数据依据", "展示行业报告、公司资料、控制优化论文和化工数字化标准依据", "04_参考文献与数据依据.docx"],
        ["方案创新与落地清单", "梳理安全、收益、责任、数据、推广和差异化能力", "05_方案创新与落地清单.docx"],
        ],
        [1.5, 2.7, 2.2],
    )

    doc.add_heading("4. 团队补充材料（链接格式）", level=1)
    add_para(doc, "GitHub 项目链接：https://github.com/zhijinDeng/synthesis-ammonia")
    add_para(doc, "链接说明：仓库包含可交互演示页面、合成氨装置数据模型、研究简报、架构说明、优化路线、Word 提交材料生成脚本和提交附件。")

    doc.add_heading("5. 队伍人数", level=1)
    add_para(doc, "1 人。")
    add_para(doc, "说明：当前材料以单人完整方案包组织，体现对行业、智能调度、工程落地和文档交付的综合能力。")
    return save(doc, "00_提交信息汇总.docx")


def build_opening_report():
    doc = setup_doc("开题报告", "命题：如何打造合成氨智能生产调度专家")
    doc.add_heading("提交正文", level=1)
    ...
    add_para(doc, PART2_SUBMISSION)
    doc.add_heading("补充展开：产业语境与系统边界", level=1)
    add_para(
        doc,
        "开题报告不能写成技术综述，而要先把产业语境讲清楚：云图的合成氨不只是单套装置稳产，而是连接尿浆、复合肥、联碱、液氨外售和氮肥自给率的经营枢纽。"
    )
    add_para(
        doc,
        "整体方案不能只写算法名词，还要交代架构完整性、创新差异、业务价值、试点路径和推广条件。因此本方案采用“影子运行先行、人机确认闭环、DCS/SIS 守底线、采纳效果进知识库”的路径，把系统定位为合成氨调度员的专业副驾驶。"
    )
    ...
    add_bullets(
        ...
        [
            ...
            "智能调度工程理解：把预测、优化、数字孪生、知识库和人机确认组合起来，而不是泛泛讲大模型。",
...
def build_solution_doc():
    doc = setup_doc("整体解决方案书", "合成氨调控师工作台：从影子运行到人机协同闭环")
    doc.add_heading("1. 方案定位", level=1)
    add_para(doc, "本方案把合成氨装置视为多约束、多目标、滚动优化的生产经营系统。系统的角色不是替代班长，而是担任“调控师副驾驶”：在安全边界内汇总数据、识别约束、生成方案、解释风险，并把采纳与偏差沉淀为知识。")
    ...
    add_table(
        ...
        [
            ...
            ["执行", "排产方案、交接摘要、复盘记录", "自动开停车或直接写控制参数"],
            ["收益", "采纳方案的实际差额收益", "把市场行情自然波动算作系统收益"],
        ],
        [1.2, 2.7, 2.6],
    )
    ...
    add_table(
        ...
        [
            ...
            ["60 天", "影子运行", "每班三套方案、未采纳原因记录", "覆盖主要场景，调度员愿意看"],
...
def build_operator_manual():
    doc = setup_doc("调控师操作手册", "合成氨智能调控师工作台：班组使用与异常处置")
    ...
    add_table(
        ...
        [
            ...
            ["低置信", "数据缺失、模型偏差或市场异常", "退回规则方案和人工流程", "不得自动回写 MES"],
        ],
        [1.1, 1.8, 2.4, 1.2],
    )
    ...
    add_numbered(
        ...
        [
            ...
            "先解释后执行：无法解释触发约束的方案不得采纳。",
            "先复盘后迭代：未采纳原因比采纳原因更能提升模型。",
        ],
    )
    doc.add_heading("6. 网页第一次使用说明", level=1)
    add_para(doc, "网页是调度工作台的演练入口，不是DCS画面。第一次使用时先点击“稳氨”，再按“当班态势—产供销平衡—稳产预警—执行复盘”的顺序查看。页面顶部的“使用说明”提供逐项组件解释，适合向不了解合成氨的人员讲解。")
    add_table(
        doc,
        ["页面组件", "它解决什么问题", "第一次怎么用", "结果如何理解"],
        [
            ["场景按钮", "回放稳态、保供、护机、错峰等班次矛盾", "点击一个场景，不要同时拖动多个滑块", "只是样例条件，不是生产命令"],
            ["沙盘滑块", "模拟订单、能源、设备、库存和行情变化", "一次调整一个变量，再看建议和事件变化", "用于验证逻辑，不代表现场实时值"],
            ["行情闸门", "区分公开参考和企业执行价格", "先读取官方参考，再由经营人员确认样例行情", "未确认时冻结新的经济排序"],
            ["24小时液氨平衡", "核对液氨从哪里来、分给谁、期末剩多少", "先看期末可用量和安全余量", "预计值不能替代罐区实测"],
            ["趋势证据", "提前提示压机等设备需要复核", "看多变量是否持续同向变化", "不等于DCS报警或SIS联锁"],
            ["动作单", "把跨装置决定拆给责任岗位", "岗位复核前置条件后记录接令", "接令不等于已执行"],
            ["飞书协同", "承接卡片、审批、任务和复盘", "先生成演示草稿，再接企业应用权限", "当前原型不代表真实写回"],
            ["证据包", "保存输入、门禁、动作和审计记录", "点击导出JSON", "用于交接和验收，不自动写入生产系统"],
        ],
        [1.25, 1.8, 2.15, 1.65],
    )
    doc.add_heading("7. 非专业人员需要先理解的术语", level=1)
    add_bullets(
        doc,
        [
            "液氨：合成氨装置的主要产品，也是尿素、复合肥、硝酸、联碱等下游装置的共同原料。",
            "负荷：装置开到多大程度；网页中的目标负荷是建议值，不是已经执行的实际值。",
            "DCS：负责装置局部调节、顺控和报警；本平台不写入DCS。",
            "SIS：负责安全联锁和保护；本平台不绕过SIS。",
            "MES/ERP：MES记录生产计划和执行事实，ERP提供订单、价格和结算口径。",
            "Historian：保存真实生产点位和历史趋势；没有接入时不填写实际负荷、能耗和收益。",
            "影子运行：平台先给建议，现场仍按原流程生产，用实际结果验证建议，再决定是否小范围执行。",
        ],
    )
    add_para(doc, "完整的逐组件说明、上手路径和常见状态处理见项目文件 docs/user-guide-for-beginners.md。")
    return save(doc, "03_调控师操作手册.docx")


def build_reference_doc():
    doc = setup_doc("参考文献与数据依据", "合成氨智能调度方案证据链")
    ...
    add_bullets(
        ...
        [
            ...
            "工业智能先进架构可作为支撑方法，但必须落到合成氨的氢氮比、合成塔、循环压缩机、液氨库存等对象上[9]。",
    ...
    add_table(
        ...
        [
            ...
            ["工业智能指南", "智能体、数字孪生、知识闭环", "作为合成氨智能调控师的技术支撑，而非泛化主题"],
...
def build_review_upgrade_doc():
    doc = setup_doc("方案创新与落地清单", "把合成氨智能调控师从演示方案升级为企业试点提案")
    doc.add_heading("1. 企业落地关键要素", level=1)
    add_table(
        doc,
        ["关键要素", "现场风险", "本方案强化点"],
        [
    ...
    add_bullets(
        doc,
        [
            "专家审核友好：页面用“调控工作说明”和“专家审核关注点”呈现待确认事项、审批责任和试点条件。",
            "反事实收益归因：同一班次保留原计划、系统方案、采纳结果和实际偏差，用反事实对比减少“把行情算成系统收益”的争议。",
            "三重护栏：工艺硬约束、安环合规护栏、飞书审批留痕同时生效，说明系统不绕开控制系统。",
    ...
    doc.add_heading("2.1 页面新增的创新落点闭环", level=2)
    add_table(
        doc,
        ["创新落点", "企业价值", "页面体现"],
        [
            ["反事实收益归因", "避免把行情变化或人工经验收益误算为系统收益", "同屏保留原计划、系统方案、采纳结果和实际执行口径"],
            ["安全护栏链", "让安环、设备、调度能看到同一套硬约束和审批边界", "工艺约束、风险指数、审批路径、停用条件联动"],
            ["未采纳学习", "把专家否决原因变成知识资产，而不是流失在口头沟通中", "未采纳原因标签进入知识库和周度校准"],
            ["协同触达", "让调度方案进入企业真实审批和交接流程", "飞书卡片、审批草稿、复盘同步和责任留痕"],
        ],
        [1.5, 2.5, 2.5],
    )

    doc.add_heading("3. 实施边界说明", level=1)
    add_table(
        doc,
        ["关注事项", "处理方式"],
        [
            ["系统是否影响安全生产", "第一阶段只做影子方案、飞书审批和复盘，不直接写 DCS/SIS，不自动开停车；安环红线接近时自动降级。"],
            ["数据不准时的处理", "先做数据就绪度评分。关键点位缺失、延迟超过 15 分钟或口径不一致时，优化方案停用，只保留人工规则提醒。"],
            ["飞书的定位", "飞书不是控制系统，而是班组协同入口。它让方案、审批、采纳、驳回、复盘都有责任人和时间戳。"],
            ["收益核算方式", "用历史回放和影子运行做对照，只认被采纳且实际执行的方案收益，并按能源、库存、订单、停车风险分类归因。"],
            ["专业性来源", "方案从合成回路、循环压缩机、合成塔床层温升、氢氮比、液氨罐区和下游用氨出发，而不是泛泛写大模型。"],
            ["能否推广？", "先沉淀合成氨约束模板、审批模板和知识库，再复制到复合肥、联碱、黄磷、磷酸铁等多装置协同场景。"],
        ],
        [1.9, 4.6],
    )

    doc.add_heading("4. 系统定位", level=1)
    add_para(
        doc,
        "平台首先呈现一个会说“我先守安全边界，再找吨氨成本”的合成氨调控师；附件材料进一步说明系统不是单一页面，而是一套能进入企业试点的运行管理系统。"
    )
    add_para(
        doc,
        "系统不承担全能接管者角色，而是作为“懂合成氨、懂班组、懂审批、懂复盘的数字调控师”。它的价值在于把专家经验从人脑、群聊和交接班口头记录中沉淀成可复盘、可审计、可迭代的企业知识资产。"
    )

    doc.add_heading("5. 依据补充", level=1)
    add_bullets(
        doc,
        [
