...
def build_opening_report():
    ...
    add_para(
        doc,
        "补充要求说明，云图合成氨装置在硬件层面的单位成本已接近理论最优，下一轮降本空间来自运行管理的软件层。命题真正考察的是能否打造“超级数字员工”：实时挖掘成本偏差，缩短调度指挥链条，沉淀专家知识并自我迭代。云图控股应城 70 万吨合成氨项目补齐氮肥原料，与尿浆、复合肥、联碱和液氨外售形成协同链路[3][4][5]。因此，AI 调度专家不应只是聊天机器人或看板，而应在不触碰 DCS/SIS 安全控制底线的前提下，融合 MES、ERP、DCS historian、罐区和设备点检数据，形成“寻优-指挥-监督-学习”闭环。"
    )
    ...
    add_para(
        doc,
        "每次方案会沉淀为知识库：输入状态、推荐方案、审批人、采纳/未采纳原因、实际执行偏差和效果归因。落地路线采用 30/60/90 天试点：30 天跑通数据事实表和口径确认，60 天进入影子模式记录采纳/不采纳原因，90 天做小闭环复盘并决定是否扩大范围。验收采用保守口径：高优订单满足率不低于人工排产，且单位氨能耗、库存占用、调度耗时或异常预警至少一项稳定改善；若关键数据延迟超过 15 分钟、模型连续两天偏差超阈值或安环红线接近且无法解释，系统自动降级为人工调度。该方案的创新点不是“让 AI 接管生产”，而是把连续流程装置的现场经验、优化模型和企业经营目标整合成可审计、可复盘、可逐步自我升级的数字员工。"
    )
    ...
    add_table(
        ...
        ["试题挑战", "方案回答", "企业可见成果"],
        [
            ["调度寻优难", "成本寻优雷达 + 三案比选 + 约束孪生", "看到每次降本来自能源、库存、设备、订单还是操作偏差"],
            ["指挥效率低", "调控师指令窗口 + 交接摘要 + 执行监督", "缩短班前会准备和跨部门确认时间"],
            ["知识传承难", "调度指令库 + 异常处置经验库 + 未采纳标签", "把专家经验变成可检索、可复盘、可迭代的知识资产"],
        ],
        [1.2, 2.6, 2.7],
    )
...
def build_solution_doc():
    ...
    doc.add_heading("3. 超级数字员工能力矩阵", level=1)
    add_table(
        doc,
        ["能力", "功能模块", "运行闭环"],
        [
            ["实时感知", "运行数据、物料成本、装置效率、库存和订单统一采集", "形成班次事实表"],
            ["成本寻优", "成本雷达、三案比选、数字孪生约束", "识别隐藏成本白银和真金"],
            ["敏捷指挥", "调控师指令窗口、交接摘要、审批链", "人工确认后下达计划"],
            ["执行监督", "负荷偏差、库存变化、订单完成、能耗结果跟踪", "自动生成复盘样本"],
            ["知识传承", "调度指令库、异常经验库、未采纳原因标签", "沉淀专家知识和场景规则"],
...
def build_reference_doc():
    ...
    add_bullets(
        ...
        [
            ...
            "工业 AI 先进架构强调基础模型、数字孪生和智能体协同，形成知识驱动仿真、仿真验证决策、决策指导执行、执行反馈知识的闭环[9]。",
            "石化和化工数字化成熟度要求强调生产平衡经验库、调度指令库、异常处置经验库、自动生成调度指令并跟踪执行结果[10]。",
        ],
    )
    ...
    add_table(
        ...
        [
            ...
            ["工业 AI 指南", "智能体、数字孪生、知识闭环", "超级数字员工和自学习迭代框架"],
            ["化工成熟度要求", "生产平衡经验库、调度指令库、执行反馈", "执行监督闭环和知识库沉淀"],
        ],
        [1.4, 2.2, 2.9],
    )
    add_references(doc)
