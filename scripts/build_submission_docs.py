...
def build_opening_report():
    ...
    add_para(
        doc,
        "该命题重点考察候选人能否把合成氨现场从“单装置稳产”抽象为“安全、能源、设备、库存、订单、环保共同约束下的经营优化”问题。云图控股应城 70 万吨合成氨项目补齐氮肥原料，与尿浆、复合肥、联碱和液氨外售形成协同链路[3][4][5]。因此，AI 调度专家不应只是聊天机器人或看板，而应在不触碰 DCS/SIS 安全控制底线的前提下，融合 MES、ERP、DCS historian、罐区和设备点检数据，给出班次级可解释排产建议、风险边界、收益测算和复盘依据。难点在于让班长、调度、设备、安环和经营部门使用同一套数据口径做决策。"
    )
    doc.add_heading("Part 2：整体解决方案设计（约 560 字）", level=2)
    add_para(
        doc,
        "方案命名为“合成氨 AI 调控师工作台”。第一阶段不自动控制装置，而是做影子运行、班前会辅助和班后复盘。平台先接入 MES 日计划、ERP 订单与交期、液氨罐区库存、DCS 关键摘要点、设备健康评分和能源价格窗口，形成班次事实表。预测层滚动预测下游需求、库存消耗、能源成本和设备风险；约束层用机理边界和专家规则校验合成塔温升、压缩机负荷、罐区压力、安全库存、负荷升降速率和环保指标；优化层生成稳态、订单冲刺、设备保守三套方案，分别说明负荷、下游分配、库存策略、点检窗口和收益风险；解释层将结果转成班长可读的交接摘要，明确“为什么这样排、触发了哪些边界、不能做什么”。"
    )
    add_para(
        doc,
        "落地路线采用 30/60/90 天试点：30 天跑通数据事实表和口径确认，60 天进入影子模式记录采纳/不采纳原因，90 天做小闭环复盘并决定是否扩大范围。验收采用保守口径：高优订单满足率不低于人工排产，且单位氨能耗、库存占用、调度耗时或异常预警至少一项稳定改善；若关键数据延迟超过 15 分钟、模型连续两天偏差超阈值或安环红线接近且无法解释，系统自动降级为人工调度。该方案的创新点不是“让 AI 接管生产”，而是把连续流程装置的现场经验、优化模型和企业经营目标整合成可审计、可复盘、可逐步推广的调度能力。"
    )
...
def build_solution_doc():
    ...
    doc.add_heading("4. 接口与数据清单", level=1)
    add_table(
        doc,
        ["系统", "首批字段", "频率", "用途"],
        [
    ...
    doc.add_heading("5. 30/60/90 天落地路线", level=1)
    add_table(
        doc,
        ["时间", "目标", "交付物", "验收口径"],
        [
    ...
    doc.add_heading("6. 验收和停用", level=1)
    add_bullets(
        doc,
        [
