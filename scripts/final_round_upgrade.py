...
PART2 = (
    ...
    "每案列明目标负荷、下游分配、边际贡献、固定成本吸收、风险指数、审批路径和回滚条件；90天只在低风险场景做小闭环验收。"
...
def build_solution_doc():
    ...
    doc.add_heading("7. 产供销经济取舍口径", level=1)
    para(
        doc,
    ...
    doc.add_heading("8. 跨装置联动的调度优先级", level=1)
    para(
        doc,
    ...
    doc.add_heading("9. 30/60/90天落地路径", level=1)
    table(
        doc,
        ["阶段", "目标", "交付物", "验收口径"],
        [
    ...
    doc.add_heading("10. 验收与停用条件", level=1)
    bullets(
        doc,
        [
...
def build_final_doc():
    ...
    doc.add_heading("五（一）边际贡献决策口径", level=2)
    para(
        doc,
    ...
    doc.add_heading("五（二）跨装置调度的三条底线", level=2)
    table(
        doc,
        ["底线", "现场含义", "平台处理方式"],
        [
    ...
    doc.add_heading("五（三）现有系统接入假设", level=2)
    table(
        doc,
        ["系统", "在现场承担的作用", "本方案调用方式"],
        [
