...
PART2 = (
    "我们建设“氨智领航调控师”，采用“事实汇总、约束校核、方案比较、人工审批、执行跟踪、班后复盘”六步流程。"
    "首先校核ERP订单导出、MES班次计划、罐区历史摘要和飞书协同记录四类离线或只读数据，形成带来源、时间戳和质量码的班次调度事实表。"
    "优化模块以安全、连续生产和设备限制为硬约束，按照液氨机会成本、非氨变动成本、物流、启停损失和违约风险比较下游分配方案，"
    "输出目标负荷、有效期、触发依据和撤销条件。建议经飞书卡片送达班组；设备趋势或安全边界触发时强制主管及相关专业会签，批准后仅写入MES计划摘要并建立跟踪任务，不写DCS/SIS。"
    "班后记录实际负荷、库存、能耗和未采纳原因，候选规则经专业人员审核后再入库。试点分三步实施：30天完成四类离线或只读数据的口径校核和接口清单，"
    "60天扩展生产历史数据库、设备和公用工程只读数据并开展旁路回放，90天选择低风险场景进行执行验证。"
)
...
def build_opening_report():
    ...
    table(
        ...
        [
            ...
            ["试点实施", "先做离线与只读数据校核，再开展旁路回放和低风险执行验证。", "30/60/90天范围、停用条件和贡献核算口径明确。"],
        ],
        [1.2, 2.75, 2.45],
    )
...
def build_solution_doc():
    ...
    table(
        ...
        [
            ["互动卡片", "指挥效率低", "把目标负荷、风险等级、触发约束、相对外售分配贡献和确认按钮推送到当班调度群。", "card_action、operator_confirm、reject_reason"],
    ...
    table(
        ...
        [
            ...
            ["90天", "开展低风险执行验证", "不少于30个有效班次，覆盖产供销重排、公辅波动吸收、设备趋势复核三类场景", "安全硬约束零突破、建议可复算、无Historian回传时实际值保持为空；收益仅与同口径原计划对照后归因。"],
        ],
        [0.75, 1.65, 2.15, 2.05],
    )
...
def build_manual_doc():
    ...
    bullets(
        doc,
        [
            "互动卡片原型：显示目标负荷、风险等级、触发约束、相对外售分配贡献、撤销条件和确认按钮。",
            "审批原型：高风险负荷调整生成审批草稿，字段包括装置、目标负荷、业务理由、风险说明和撤销条件。",
    ...
    table(
        ...
        ["操作节点", "飞书动作", "班组确认口径"],
        [
            ["方案生成", "企业发布后，调控师向合成氨当班群发送互动卡片。", "核对目标负荷、触发约束、风险等级、撤销条件和与原计划差异。"],
...
def build_reference_doc():
    ...
    bullets(
        ...
        [
            ...
            "企业试点从离线和只读数据校核开始，经过旁路回放、人工确认和停用条件验证后，再进入低风险执行验证。",
        ],
    )
    add_references(doc)
...
def build_innovation_doc():
    ...
    table(
        ...
        [
            ...
            ["实施与治理", "已形成数据契约、控制边界、30/60/90天范围、停用条件和复盘字段。", "完成四类首批数据校核、旁路回放、低风险执行验证和验收基线确认。"],
        ],
        [1.2, 2.35, 2.55],
    )
    ...
    table(
        ...
        [
            ...
            ["飞书协同", "区分已验证链路、演示原型和待授权能力。", "Task v2任务、Base样例、卡片与审批原型、Aily和事件回传授权清单。"],
...
def build_final_doc():
    ...
    table(
        ...
        [
            ...
            ["使用的飞书AI能力", "已验证飞书Task v2与多维表格样例；互动卡片和审批为原型；飞书智能问答与事件回传待授权。"],
        ],
        [1.45, 4.95],
    )
    ...
    table(
        ...
        ["飞书能力", "业务动作", "需要授权"],
        [
            ["互动卡片", "推送目标负荷、风险等级、触发约束和确认按钮。", "机器人发消息、目标群可见权限。"],
    ...
    para(doc, f"飞书交付状态分为三类。已验证：在线方案稿 {FEISHU_DOC_URL}、多维表格样例库 {FEISHU_BASE_URL}、Task v2任务清单 {FEISHU_TASK_URL} 和验收任务 {FEISHU_VERIFIED_TASK_URL}。演示原型：互动卡片、审批状态机和事件回传字段契约。待企业授权：目标群机器人、审批定义、Aily知识源、事件订阅及MES和生产历史数据库接口。")
    ...
    para(doc, "演示流程控制在3-5分钟：先展示当班建议、有效期和24小时液氨平衡；再逐项复算相对外售分配贡献；随后演示设备趋势证据、审批状态机、飞书Task v2验收任务与多维表格复盘链路。滑块只用于场景回放，不代表企业实时数据。")
    doc.add_heading("九、自由展示区", level=1)
    bullets(doc, [
        "项目已形成可交互页面、可复现生成脚本、数据模型、Word提交材料和GitHub开放仓库。",
        "材料使用班组可核对的业务对象、数据来源和操作边界描述平台能力。",
        "交付状态按已验证、演示原型和待企业授权三类标识，生产效果以试点数据为准。",
    ])
...
def build_feishu_module_doc():
    ...
    para(
        doc,
        "飞书承接调度建议送达、人工确认、任务跟踪和班后复盘。优化模块生成目标负荷、风险等级、相对外售分配贡献和撤销条件；飞书保存负责人、审批状态、截止时间和执行结果。班后记录经专业人员审核后才进入案例库。",
    )
    ...
    table(
        ...
        ["方向", "飞书动作", "沉淀结果"],
        [
            ["产供销协同", "卡片推送液氨去向、相对外售分配贡献、负荷调整、外采和停运评估；高风险建议进入审批。", "调整原因、确认人、测算值、实际结果和未采纳原因。"],
            ["跨装置协调", "下游需求、能源、公用工程和库存变化触发复核任务。", "波动来源、协调动作、执行偏差和撤销条件。"],
    ...
    table(
        ...
        [
            ...
            ["指挥效率低", "互动卡片送达当班群，审批承接高风险调整，任务清单记录执行与复盘。", "保存确认人、审批人、负责人、截止时间和关闭说明。"],
    ...
    table(
        ...
        [
            ["互动卡片", "产供销、跨装置协调和设备趋势方案", "目标负荷、风险等级、触发约束、撤销条件、采纳/复核/驳回按钮", "只触发流程，不直接控制装置。"],
    ...
    numbers(
        ...
        [
            ...
            "执行监督：企业发布后，由审批结果创建飞书任务，跟踪生产历史数据库中的实际负荷、库存和异常处置。",
    ...
    bullets(
        doc,
        [
            "字段样例：data/feishu_dispatch_review_template.csv 保存班次、场景、目标负荷、风险等级、24小时相对外售分配贡献、采纳状态和复盘备注等字段。",
    ...
    bullets(
        ...
        [
            ...
            "待企业授权：目标群机器人范围、审批定义approval_code、Aily知识源、事件订阅、回调地址、密钥托管及企业系统接口。",
        ],
    )
