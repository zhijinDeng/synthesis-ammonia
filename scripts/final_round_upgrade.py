...
def build_solution_doc():
    ...
    para(
        doc,
        "根据企业访谈记录，现场已有或正在建设多套生产与经营系统。平台据此定位为现有商业软件之上的跨系统调度编排层，不重做DCS、MES、APC或压机软件，而是理清各系统主要功能、数据流向和写回边界。",
    )
    table(
        doc,
    ...
    bullets(
        ...
        [
            ...
            "机理与模型侧：先以合成塔热平衡、反应器温升、氢氮比、循环气量和压缩机效率形成硬约束；PINN只有完成企业历史回放与留出班次验证后，才可作为影子计算的候选校准方法。",
    ...
    para(
        doc,
        "企业访谈显示，当前MES、DCS、IoT、APC、压机软件、热电平台和经营数据之间仍需人工串联。真正困难的不是某一个系统有没有数据，而是调度长需要从多套系统抽取信息，再把价格、库存、订单、装置负荷、设备状态和公辅约束汇总判断。成熟调度长培养周期可达五年，这正是数字调度长需要补位的地方。",
    )
    table(
        doc,
    ...
    para(
        doc,
        "产供销调度不能简单按“售价高于完全成本才生产”判断。验收账本采用增量口径：产品净收入减非氨变动成本、包装物流、液氨机会成本和启停摊销，再加可核验的违约避免额。折旧、固定人工等只作为管理解释，不重复加入增量贡献。若贡献为负且占用稀缺液氨、能源或关键设备能力，则进入降负荷或停产评估。",
    )
    table(
        doc,
        ["测算项", "现场含义", "调控师使用方式"],
    ...
    table(
        ...
        ["阶段", "目标", "交付物", "验收口径"],
        [
            ["30天", "完成首批四类数据口径", "ERP订单导出、MES班次计划、罐区历史摘要、飞书协同记录的字段字典与质量报告", "调度员能逐字段核对来源、时间戳、质量码和责任专业。"],
...
def build_manual_doc():
    ...
    numbers(
        ...
        [
            ...
            "取舍先算重启代价：确需停系统时，必须比较空分、合成气轮机驱动系统和下游装置的重启时间、废料、人员与物料损失；既往口述案例只作提示，最终由班长、调度主管和相关专业会签。",
...
def build_innovation_doc():
    ...
    table(
        ...
        ["能力", "现有交付", "下一步验收"],
        [
            ["业务结构", "三大挑战、三案、30/60/90天路径已经形成。", "以产供销平衡为主线，用稳产预警和知识沉淀支撑。"],
...
def build_final_doc():
    ...
    para(doc, "本方案已形成可本地打开的调控师工作台、数据模型、架构文档、飞书协同设计、操作手册和参考文献。平台以产供销协同与跨装置负荷联动为主线，以稳定生产波动吸收和设备弱信号提示为支撑，并用班次复盘沉淀调度经验。")
    ...
    table(
        ...
        [
            ...
            ["约束与候选模型", "机理硬约束、APC/RTO接口、MPC建议；PINN仅作待验证校准候选", "先证明边界、适用域与退出条件，再进入影子计算。"],
    ...
    para(
        doc,
        "产供销协同的核心不是简单比较售价与完全成本。平台采用可复算的增量口径：产品净收入减非氨变动成本、包装物流、液氨机会成本和启停摊销，再加可核验的违约避免额。折旧和固定人工等固定成本吸收只作管理解释，不与增量贡献重复相加。",
    )
    table(
        doc,
        ["判断项", "为什么重要", "平台输出"],
    ...
    para(doc, "演示流程控制在3-5分钟：先展示当班建议、有效期和24小时液氨平衡；再进入产供销页逐项复算贡献账；随后演示弱信号证据、审批状态机、飞书真实验收任务与Base复盘链路。滑块只用于场景回放，不代表企业实时数据。")
    doc.add_heading("九、自由展示区", level=1)
    bullets(doc, [
        "项目已形成可交互页面、可复现生成脚本、数据模型、Word提交材料和GitHub开放仓库。",
        "方案语言保持企业现场视角，聚焦装置运行、班组协同、收益核算和知识沉淀。",
        "交付状态按已验证、原型和待企业联调三类标识，避免把设计能力写成已经上线。",
    ])
...
def build_feishu_module_doc():
    ...
    bullets(
        doc,
        [
            "字段样例：data/feishu_dispatch_review_template.csv 保存班次、场景、目标负荷、风险指数、24小时增量贡献、采纳状态和复盘备注等字段。",
    ...
    bullets(
        ...
        [
            ...
            "飞书任务API验收任务（已验证）：https://applink.feishu.cn/client/todo/detail?guid=44631b59-b834-47b1-a413-b751f2f291da&suite_entity_num=t136777",
            "在线Base与验收任务为已验证链路；互动卡片、负荷审批和事件回调为原型，企业发布仍需目标群机器人范围、审批定义 approval_code、知识源、回调地址和密钥托管。原生任务AI智能体需平台侧开放相应能力，密钥不写入仓库。",
        ],
    )
...
def build_submission_summary():
    ...
    table(
        ...
        ["05_方案创新与落地清单.docx", "关键创新点、现有交付状态和上线清单。"],
    ...
    <main class="shell">
      ...
      <section class="stage">
        <section class="source-strip" aria-label="数据接入状态">
          <span><i class="pulse good"></i>IoT样例延迟 18秒</span>
          <span><i class="pulse good"></i>DCS摘要样例延迟 45秒</span>
          <span><i class="pulse good"></i>热电APC样例延迟 2分钟</span>
          <span><i class="pulse good"></i>MES样例延迟 4分钟</span>
          <span><i class="pulse warn"></i>行情样例延迟 23分钟</span>
        ...
        <section class="grid-2">
          ...
          <section class="panel">
<div class="panel-title"><span>方案依据与适用范围</span><strong id="modelTrust">候选校准方法</strong></div>
...
{ title: "反应器候选校准", body: "PINN仅作为候选方法；须完成企业历史回放与留出班次验证后才能参与影子计算。", tag: "尚未企业校准" },
...
def write_data_and_docs():
    model = {
        ...
        "delivery_status_note": "网页数值为验收样例；企业点位、阈值、价格和成本口径尚待联调。",
        "final_priority_lanes": FINAL_PRIORITY_LANES,
        "units": [
            {"id": "air_separation", "name": "空分", "key_constraints": ["氧氮供应", "压缩机余量", "停车重启时间", "与气化负荷匹配"]},
            {"id": "gasification", "name": "气化", "key_constraints": ["原料与氧气稳定", "蒸汽平衡", "粗煤气流量", "最低稳定负荷"]},
            {"id": "purification", "name": "净化/变换", "key_constraints": ["CO2分离", "硫含量", "氢氮比", "压降"]},
            {"id": "compression", "name": "合成气/循环气压缩", "key_constraints": ["振动", "轴位移", "防喘振裕度", "轴承温度"]},
            {"id": "synthesis", "name": "氨合成回路", "key_constraints": ["合成塔床层温升", "循环压缩机负荷", "最低稳定负荷", "升降负荷速率"]},
            {"id": "refrigeration", "name": "冷冻与氨分离", "key_constraints": ["冷量", "压力", "液位", "氨分离效率"]},
        ...
        "decision_hierarchy": [
            ...
            {"priority": 3, "name": "特殊情况取舍", "rule": "确需中断时比较空分、合成气轮机驱动系统和下游装置的重启时间、废料、人员物料成本和影响范围；既往案例不固化为通用停机规则。"},
        ],
        "economic_decision_rules": [
            {"name": "边际贡献", "rule": "销售收入扣除原料、蒸汽、电力、包装、物流等变动成本后仍为正时，可作为继续生产的重要理由。"},
            {"name": "固定成本吸收", "rule": "作为管理解释单列展示，不重复计入增量贡献；继续生产仍需同时满足安全、连续和机会成本条件。"},
        ...
        "operator_cockpit": {
            ...
            "execution_supervision": ["班长确认", "目标负荷", "现场执行反馈", "效果复核"],
        ...
        "feishu_ai": {
            ...
            "actions": ["生成班组卡片草稿", "生成执行任务草稿", "保存复盘草稿"],
            "capability_status": {"base": "online_verified", "verification_task": "online_verified", "interactive_card": "prototype", "approval": "prototype", "task_ai_agent": "platform_enablement_required"},
            "real_artifacts": {"document": FEISHU_DOC_URL, "base": FEISHU_BASE_URL, "tasklist": FEISHU_TASK_URL, "verification_task": "https://applink.feishu.cn/client/todo/detail?guid=44631b59-b834-47b1-a413-b751f2f291da&suite_entity_num=t136777"},
        },
        "data_contract": ["source_system", "source_timestamp", "ingested_at", "unit", "quality_code", "owner", "lineage_id"],
        "recommendation_contract": ["recommendation_id", "input_snapshot_hash", "rule_version", "model_version", "valid_until", "approval_path", "rollback_condition", "idempotency_key"],
        "interfaces": [
...
项目重心是产供销全线平衡和孤岛系统贯通：把MES、IoT、DCS、APC、压机、热电和经营信息汇成可追溯的班次事实表，再用经专家审核的案例库缩短新人从跟班到独立判断的成长周期。首批不追求全系统一次打通，而是从订单、班次计划、罐区摘要和飞书协同记录开始验证。
"""
...
def main():
    if "--regenerate-ui" in sys.argv:
        print("UI files are maintained directly; --regenerate-ui is intentionally disabled.")
    write_data_and_docs()
    paths = [
